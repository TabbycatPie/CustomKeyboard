...
/************** 键盘输入端口描述符(端点描述符) *****************/
	...
	0x08,	/* 该端点支持的最大包长度 */
	0x00,
	0x06,	/* 轮询间隔 bInterval: Polling Interval (6 ms)*/
...
//	0x07,	/* 该端点支持的最大包长度 */
//	0x00,
//	0x0a    /* 轮询间隔 bInterval: Polling Interval (32 ms)*/	
...
//	SPI_WRITE_BUF(W_REGISTER + TX_ADDR,   (unsigned char*)TX_ADDRESS_DEF1, 5);
//	SPI_Read_Buf(TX_ADDR, TX_ADDRESS_DEF2, 5);
//	if(TX_ADDRESS_DEF2[0] == 0x01 && TX_ADDRESS_DEF2[1] == 0x01 && TX_ADDRESS_DEF2[2] == 0xA0 && TX_ADDRESS_DEF2[3] == 0x11 &&  TX_ADDRESS_DEF2[4] == 0x00) 
//	{
//		while(1)
//		{
//			if(Ready)
    ...
    while(1)
    {
		...
		if(Ready)
		{
			...
			if(sta == RX_DR_FLAG)
			{
				...
				//读出接收数据
				SPI_Read_Buf(R_RX_PAYLOAD, ucPayload, 8);
				//清空接收FIFO
				SPI_Write(FLUSH_RX, 0);
				
				//处理
				if(ucPayload[0] == 0xaa && ucPayload[1] == 0x01 && ucPayload[2] == 0x02)
				{
					HIDValueHandle('Q');
					ucPayload[0] = 0x00;
				}
				...
				//清中断标志 
				SPI_Write(W_REGISTER + STATUS, 0x70);
